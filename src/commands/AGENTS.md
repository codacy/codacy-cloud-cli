...
- **Dependency import chains** (SCA issues): when `issue.dependencyChains` is present, the detail view renders it the same way `finding`'s detail does — see "Dependency import chains" under `finding` below

## issues command (`issues.ts`)

- Takes `<provider>`, `<organization>`, and `<repository>` as required arguments
- **List mode** (default): card-style format sorted by severity (Error > High > Warning > Info). When `issue.dependencyChains` is present (SCA issues), the card shows the same one-line dependency-chain summary `findings`' list uses — see "Dependency import chains" under `finding` below. `IgnoredIssue` (the `--ignored` variant) has no `dependencyChains` field, so ignored-issue cards don't render it
